...
class UpdateLocationDialogFragmentIntegrationTest : BaseFragmentTests() {
    ...
    fun given_update_location_dialog_when_rotate_then_alert_dialog_shown_again() {
        ...
        uiDevice.wait(Until.hasObject(By.desc(UpdateLocationDialogFragment.UPDATE_LOCATION_MAP_FINISH_LOADING)),
            20000)
    ...
    fun given_update_location_dialog_and_search_is_launched_when_click_on_us_address_then_us_address_is_correctly_display_on_details_party() {
        ...
        onView(withId(R.id.street)).check(matches(withText(fakePlaceDetails.addressComponents.single { addressComponent ->
           addressComponent.types.contains(Place.Type.ROUTE.name.lowercase(Locale.getDefault()))
        }.longName)))
        onView(withId(R.id.city)).check(matches(withText(fakePlaceDetails.addressComponents.single { addressComponent ->
            addressComponent.types.contains(Place.Type.LOCALITY.name.lowercase(Locale.getDefault()))
        }.longName)))
        onView(withId(R.id.postal_code)).check(matches(withText(fakePlaceDetails.addressComponents.single { addressComponent ->
            addressComponent.types.contains(Place.Type.ADMINISTRATIVE_AREA_LEVEL_1.name.lowercase(Locale.getDefault()))
        }.shortName)))
        onView(withId(R.id.country)).check(matches(withText(fakePlaceDetails.addressComponents.single { addressComponent ->
            addressComponent.types.contains(Place.Type.COUNTRY.name.lowercase(Locale.getDefault()))
        }.longName)))
        onView(withId(R.id.state)).check(matches(withText(fakePlaceDetails.addressComponents.single { addressComponent ->
            addressComponent.types.contains(Place.Type.ADMINISTRATIVE_AREA_LEVEL_1.name.lowercase(Locale.getDefault()))
        }.longName)))
    }

...
class AddPhotoDialogFragmentIntegrationTest : BaseFragmentTests() {
    ...
    fun given_add_photo_dialog_and_values_completed_when_rotate_then_same_values_displayed() {
        ...
        val bitmap = (updateFragment.addPhotoAlertDialog.binding.photoImageview.drawable as BitmapDrawable).bitmap
    ...
    fun given_add_photo_dialog_when_select_image_from_gallery_then_return_result_with_success() {
        ...
        onView(withId(R.id.select_photo_from_gallery))
            .perform(click())
        assertThat(updateFragment.addPhotoAlertDialog.latestTmpUri).isSameInstanceAs(expectedResult)
    }
    ...
    fun given_add_photo_dialog_shown_when_take_a_photo_then_return_result_with_success() {
        ...
        val file = File(InstrumentationRegistry.getInstrumentation().targetContext.cacheDir, updateFragment.addPhotoAlertDialog.latestTmpUri!!.lastPathSegment!!).absolutePath
        try {
    ...
    fun given_add_photo_dialog_when_take_a_photo_then_tmp_file_created() {
        ...
        assertThat(updateFragment.addPhotoAlertDialog.tmpFile).isNotNull()

        val storedFile = File(InstrumentationRegistry.getInstrumentation().targetContext.cacheDir, updateFragment.addPhotoAlertDialog.latestTmpUri!!.lastPathSegment!!)

        assertThat(storedFile).isNotNull()
        assertThat(storedFile).isEqualTo(updateFragment.addPhotoAlertDialog.tmpFile)
    }
    ...
    fun given_add_photo_dialog_and_tmp_file_created_when_click_on_add_photo_button_then_tmp_file_deleted() {
        ...
        val file = File(InstrumentationRegistry.getInstrumentation().targetContext.cacheDir, updateFragment.addPhotoAlertDialog.latestTmpUri!!.lastPathSegment!!)
    ...
    fun given_add_photo_dialog_and_photo_selected_when_click_on_add_photo_button_then_tmp_file_saved_with_other_photos() {
        ...
        val tmpFile = File(InstrumentationRegistry.getInstrumentation().targetContext.cacheDir, updateFragment.addPhotoAlertDialog.latestTmpUri!!.lastPathSegment!!)
    ...
    fun given_add_photo_dialog_and_photo_added_when_add_photo_button_then_photo_at_position_in_recycler_view_not_empty() {
        ...
        onView(withId(R.id.photos_recycler_view)). perform(
            scrollToPosition<PhotoUpdateAdapter.PhotoViewHolder>(updateFragment.binding.photosRecyclerView.adapter!!.itemCount - 1)
        )

        val viewHolder = updateFragment.binding.photosRecyclerView
            .findViewHolderForAdapterPosition(
                updateFragment.binding.photosRecyclerView.adapter!!.itemCount - 1
    ...
    fun given_add_photo_dialog_when_click_on_delete_photo_then_photo_deleted() {
        ...
        onView(withId(R.id.add_a_photo)).perform(click())
        if(updateFragment.addPhotoAlertDialog.binding.photoImageview.drawable == null) {
            onView(withId(R.id.take_photo)).perform(click())
        }

        onView(withId(R.id.delete_photo)).perform(click())
        assertThat(updateFragment.addPhotoAlertDialog.binding.photoImageview.drawable).isNull()
    }
    ...
    fun given_add_photo_dialog_when_click_on_main_photo_then_main_photo_change() {
        ...
        val oldMainPhotoViewHolder = updateFragment.binding.photosRecyclerView
    ...
    fun given_add_photo_dialog_when_add_photo_then_changes_occurs_in_photo_recycler_view() {
        ...
        val updateDialogPhotoBitmap = (updateFragment.addPhotoAlertDialog.binding.

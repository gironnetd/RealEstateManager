...
class PhotoRemoteDataSourceTest : TestCase() {
    ...
    public override fun setUp() {
        ...
        remoteDataSource = PhotoRemoteDataSource(firestore = firestore, storage = null)
    ...
    public override fun tearDown() {
        remoteDataSource.deleteAllPhotos().blockingAwait()
    ...
    fun given_remote_data_source_when_save_photos_then_counted_successfully() {
        ...
        remoteDataSource.savePhotos(fakePhotos).blockingAwait()

        // Then count of photos in database is equal to given photos list size
        assertThat(remoteDataSource.count().blockingGet()).isEqualTo(fakePhotos.size)
    }

    @Test
    fun given_remote_data_source_when_save_photos_then_counted_by_propertyId_successfully() {
        // Given photos list and When photos list saved
        remoteDataSource.savePhotos(fakePhotos).blockingAwait()

        // Then count of photos in database is equal to given photos list size
        assertThat(remoteDataSource.count(firstPropertyId).blockingGet())
            .isEqualTo(fakePhotos.subList(0, fakePhotos.indices.count() / 2).size)

        assertThat(remoteDataSource.count(secondPropertyId).blockingGet())
            .isEqualTo(fakePhotos.subList(fakePhotos.indices.count() / 2, fakePhotos.indices.count()).size)
    }

    @Test
    fun given_remote_data_source_when_save_a_photo_then_saved_successfully() {
        // Given photos list and When photos list saved
        remoteDataSource.savePhoto(fakePhotos[0]).blockingAwait()

        // Then count of photos in database is equal to given photos list size
        assertThat(remoteDataSource.findPhotoById(fakePhotos[0].id).blockingGet()).isEqualTo(fakePhotos[0])
    }

    @Test
    fun given_remote_data_source_when_save_photos_then_saved_successfully() {
        // Given photos list and When photos list saved
        fakePhotos = fakePhotos.sortedBy { it.id }
        remoteDataSource.savePhotos(fakePhotos).blockingAwait()

        // Then returned photos in database is equal to given photos list
        assertThat(remoteDataSource.findAllPhotos().blockingGet()).isEqualTo(fakePhotos)
    }

    @Test
    fun given_remote_data_source_when_find_all_photos_then_found_successfully() {
        // Given photos list
    ...
    @Test
    fun given_remote_data_source_when_find_photo_by_id_then_found_successfully() {
        remoteDataSource.savePhotos(fakePhotos).blockingAwait()
        val photo = fakePhotos[fakePhotos.indices.random()]
        val expectedPhoto: Photo = remoteDataSource.findPhotoById(photo.id).blockingGet()
        assertThat(expectedPhoto).isEqualTo(photo)
    }

    @Test
    fun given_remote_data_source_when_find_photos_by_ids_then_found_successfully() {
        remoteDataSource.savePhotos(fakePhotos).blockingAwait()
        ...
        var expectedPhotos: List<Photo> = remoteDataSource.findPhotosByIds(photoIds).blockingGet()
    ...
    @Test
    fun given_remote_data_source_when_update_photo_then_updated_successfully() {
        ...
        remoteDataSource.updatePhoto(updatedPhoto).blockingAwait()

        val finalPhoto = remoteDataSource.findPhotoById(initialPhoto.id).blockingGet()
        assertThat(finalPhoto).isEqualTo(updatedPhoto)
    }

    @Test
    fun given_remote_data_source_when_update_photos_then_updated_successfully() {
        ...
        remoteDataSource.updatePhotos(updatedPhotos).blockingAwait()

        val ids = initialPhotos.map { photo -> photo.id }
        val finalPhotos = remoteDataSource.findAllPhotos().blockingGet().filter {
    ...
    @Test
    fun given_remote_data_source_when_delete_photo_by_id_then_deleted_successfully() {
        remoteDataSource.savePhotos(fakePhotos).blockingAwait()

        assertThat(remoteDataSource.findAllPhotos().blockingGet().size).isEqualTo(fakePhotos.size)
        val photo = fakePhotos[fakePhotos.indices.random()]
        remoteDataSource.deletePhotoById(photo.id).blockingAwait()
        assertThat(remoteDataSource.findAllPhotos().blockingGet().contains(photo))
            .isFalse()
    }

    @Test
    fun given_remote_data_source_when_delete_photos_by_ids_then_deleted_successfully() {
        remoteDataSource.savePhotos(fakePhotos).blockingAwait()
        ...
        remoteDataSource.deletePhotosByIds(photoIds).blockingAwait()

        val findAllPhotos = remoteDataSource.findAllPhotos().blockingGet()
    ...
    @Test
    fun given_remote_data_source_when_delete_photos_then_deleted_successfully() {
        remoteDataSource.savePhotos(fakePhotos).blockingAwait()
        assertThat(remoteDataSource.findAllPhotos().blockingGet().size).isEqualTo(fakePhotos.size)

        remoteDataSource.deletePhotos(fakePhotos.subList(0, 2)).blockingAwait()
    ...
    @Test
    fun given_remote_data_source_when_delete_all_photos_then_deleted_successfully() {
        remoteDataSource.savePhotos(fakePhotos).blockingAwait()
        assertThat(remoteDataSource.findAllPhotos().blockingGet().size
        ).isEqualTo(fakePhotos.size)
        remoteDataSource.deleteAllPhotos().blockingAwait()
        assertThat(remoteDataSource.findAllPhotos().blockingGet()).isEmpty()
    }
}